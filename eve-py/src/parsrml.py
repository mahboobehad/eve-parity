...
def p_input_1(p):
    ...
    modules.append(
        {1: {p[2]}, 2: set(controlledVariables), 3: {str(initCommands)}, 4: {str(updateCommands)}, 5: {p[10]},
         6: set(alphabets)})
...
def p_input_5(p):
    '''input : input PAYOFFS payoff_list'''


def p_payoff_list(p):
    '''payoff_list : payoff_tuple
                   | payoff_list payoff_tuple'''


def p_payoff_tuple(p):
    '''payoff_tuple : DCOL LB NAME COMMA NAME COMMA NUMBER RB SEMICOLON'''
    global payoffs
    payoffs.append(
        {
            'module': p[3],
...
def p_payoff_error(p):
    '''payoff_tuple : DCOL LB error COMMA NAME COMMA NUMBER RB SEMICOLON'''
    print("Syntax error in payoff tuple")
    p[0] = None
...
def parse_payoffs(data):
    global payoffs
    payoffs = []  # Reset payoffs
    parser.parse(data)
    return payoffs
...
if __name__ == "__main__":
    ...
    for payoff in payoffs:
        print(f"Module: {payoff['module']}, State: {payoff['state']}, Value: {payoff['value']}")
